...
public class GraphVisualizer {
    ...
    private void nodesByCluster(StringBuilder sb, boolean doCtrl, Collection<Node> all) {
        ...
        for (Node n : all) {
            ...
            if( _separateControlCluster && !doCtrl &&  (n instanceof CFGNode) ) continue;
            sb.append("\t\t").append(n.dotuniqueName()).append(" [ ");
            String lab = n.dotlabel();
            if (n instanceof MultiNode) {
                ...
                for (Node use : n._outputs) {
                    if (use instanceof ProjNode proj) {
                        ...
                        sb.append("<TD PORT=\"p").append(proj._idx).append("\">").append(proj.dotlabel()).append("</TD>");
                    }
                    if (use instanceof CProjNode proj) {
                        ...
                        sb.append("<TD PORT=\"p").append(proj._idx).append("\" BGCOLOR=\"yellow\">").append(proj.dotlabel()).append("</TD>");
                    }
                }
                if (doProjTable) {
                    sb.append("</TR>").append("\n");
        ...
        if (!_separateControlCluster) {
            ...
            for (Node n : all) {
                if (n instanceof RegionNode region) {
                    sb.append("\t\t{ rank=same; ");
                    sb.append(region.dotuniqueName()).append(";");
                    for (Node phi : region._outputs)
                        if (phi instanceof PhiNode) sb.append(phi.dotuniqueName()).append(";");
                    sb.append("}\n");
                }
            }
        }
    ...
    private void scope( StringBuilder sb, ScopeNode scope ) {
        ...
        for( int i = 0; i < max; i++ ) {
            ...
            if( level==0 ) {
                ...
                sb.append("\t\t").append(n.dotuniqueName()).append(" [label=\"").append(n.dotlabel()).append("\"];\n");
            }
    ...
    private String makeScopeName(ScopeNode sn, int level) { return sn.dotuniqueName() + "_" + level; }
    ...
    private void scopeEdges( StringBuilder sb, ScopeNode scope ) {
        ...
        for( var v : scope._vars ) {
            ...
            sb.append("\t")
                ...
                .append(" -> ");
            if( def instanceof CProjNode proj ) {
                sb.append(def.in(0).dotuniqueName()).append(":p").append(proj._idx);
            } else if( def instanceof ProjNode proj ) {
                sb.append(def.in(0).dotuniqueName()).append(":p").append(proj._idx);
            } else sb.append(def.dotuniqueName());
            sb.append(";\n");
     ...
     */
    private static void walk(TreeMap<Integer,Node> all, Node n) {
        if(n == null ) return;
